    ...
    **Mandatory Equations**
    ...
    def simplify_pressure_enthalpy_mass_topology(self, inconni):
        if self.simplify_pressure_enthalpy_mass_topology_check():
            ...
            for conn in [self.outl[0], self.outl[2]]:
                if conn.h.is_set:
            ...
            # contain all enthalpy shared connection within the system
            if conn_h_shared_container:
                all_sys_conn_h_shared_list = list(set([c for c_shared in conn_h_shared_container
                                                        for c in c_shared.h.shared_connection]
                                                        + [self.outl[0], self.outl[2]]))
            else:
                all_sys_conn_h_shared_list = [self.outl[0], self.outl[2]]
