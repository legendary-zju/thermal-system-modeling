        ...
        for fluid in self.variable_fluids:
            for o in self.outl:
                if self.is_variable(o.m):
                    self.network.jacobian[k, o.m.J_col] = -o.fluid.val[fluid]
                if fluid in o.fluid.is_var:
                    self.network.jacobian[k, o.fluid.J_col[fluid]] = -o.m.val_SI

            if self.is_variable(i.m):
                self.network.jacobian[k, i.m.J_col] = i.fluid.val[fluid]
            if fluid in i.fluid.is_var:
                self.network.jacobian[k, i.fluid.J_col[fluid]] = i.m.val_SI
    ...
    def energy_balance_deriv(self, increment_filter, k):
        ...
        for o in self.outl:
            if self.is_variable(i.p):
                self.network.jacobian[k, i.p.J_col] = dT_dp_in
            if self.is_variable(i.h):
                self.network.jacobian[k, i.h.J_col] = dT_dh_in
            # for fluid in i.fluid.is_var:
            #     self.jacobian[k, i.fluid.J_col[fluid]] = dT_dfluid_in[fluid]
            args = (o.p.val_SI, o.h.val_SI, o.fluid_data, o.mixing_rule)
            self.network.jacobian[k, o.p.J_col] = -dT_mix_dph(*args)
            self.network.jacobian[k, o.h.J_col] = -dT_mix_pdh(*args)
