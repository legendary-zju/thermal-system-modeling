...
from Aurora import __datapath__
from Aurora.tools import logger
from Aurora.tools.global_vars import ERR
from Aurora.tools.global_vars import fluid_property_data
from Aurora.tools.data_containers import Constraints as dc_cons


def get_all_subdictionaries(data):  # data: branches/subbranches dict
    subdictionaries = []
    for value in data.values():  # value: single branch dict
        if len(value["subbranches"]) == 0:
            subdictionaries.append(
                {k: v for k, v in value.items() if k != "subbranches"}
            )
        else:
            subdictionaries.append({k: v for k, v in value.items() if k != "subbranches"})  #
            subdictionaries.extend(get_all_subdictionaries(value["subbranches"]))  # plot construction
...
class AURORANetworkError(Exception):
    """Custom message for network related errors."""

    pass


class AURORAConnectionError(Exception):
    """Custom message for connection related errors."""

    pass


class AURORAComponentError(Exception):
    """Custom message for component related errors."""

    pass


def vectorize_conversion(func):
    """
    ...
    :return:
    """
    def wrapper(property, value, unit):
        """
        Wrapper function for vectorized functions.

        :param property:
        :param value: int, float, list, np.ndarray, tuple
        :param unit:
        :return:
        """
        # one dimensional type
        if isinstance(value, (int, float)):
            return func(property, value, unit)
        # multiple dimensional type
        value_array = np.asarray(value)
        result = np.vectorize(lambda x: func(property, x, unit))(value_array)
        # reversion value type
...
def convert_to_SI(property, value, unit):
    ...
    ----------
    property : dict
        Contain the information about the property.
    ...
    """
    if property['type'] == 'linear':
        converters = property['units'][unit]
        return (value + converters[0]) * converters[1]
    elif property['type'] == 'ratio':
        return value * property['units'][unit]
    else:
        msg = f"Has no defined unit type: {property.type} '{unit}'"
...
def convert_from_SI(property, SI_value, unit):
    ...
    if property['type'] == 'linear':
        ...
        return SI_value / property['units'][unit]
    else:
        msg = f"Has no defined unit type: {property.type} '{unit}'"
        raise NotImplementedError(msg)
...
class UserDefinedEquation:  # never benn used
    ...
    def __init__(self, label, func, deriv, conns, params={},
        ...
        >>> from Aurora.components import Source, Sink, Pipe
        >>> from Aurora.networks import Network
        >>> from Aurora.connections import Connection
        >>> from Aurora.tools import UserDefinedEquation
        >>> from Aurora.tools import CharLine
        >>> from Aurora.tools.fluid_properties import T_mix_ph, v_mix_ph
        ...
        if isinstance(conns, list):
            ...
            msg = (
                ...
                'Aurora.connections.connection.Connection objects.')
            logger.error(msg)
            raise TypeError(msg)
    ...
    def user_defined_deriv(self, increment_filter, k):
        self.network.jacobian[k, ] = self.deriv(self)

    def solve(self):
        equation_container = dc_cons(func=self.user_defined_func,
                                     deriv=self.user_defined_deriv,
                                     constant_deriv=False,
                                     num_eq=1,
                                     scale=self.scale)
        equation_container.label = f'User defined equation: {self.label}: None'
        self.network.sorted_equations_objective[self.network.equations_index] = equation_container
        self.network.residual_scale[self.network.equations_index: self.network.equations_index + equation_container.num_eq] = \
            [equation_container.scale for _ in range(equation_container.num_eq)]
        self.network.equations_index += equation_container.num_eq
...
def _numeric_deriv(obj, func, dx, conn=None, **kwargs):
    ...
    if conn is None:
        ...
        obj.get_attr(dx).val += d  # restore the value of data container
...
def bus_char_evaluation(component_value, char_func, reference_value, bus_value, **kwargs):
    r"""
    Calculate the residual value of a bus.

    Parameters
    ----------
    bus_value : float
        Value of offdesign component bus.
    ...
    char_func : Aurora.tools.characteristics.char_line
...
def bus_char_derivative(component_value, char_func, reference_value, bus_value, **kwargs):
    ...
    d = 1e-3
    """ use discrete derivatives instead of continuous derivatives, since the charline is interpolation function """
    return (1 - (
        1 / char_func.evaluate((bus_value + d) / reference_value) -
        1 / char_func.evaluate((bus_value - d) / reference_value)  # char_func.evaluate: calculate the relative ratio of design/offdesign
    ) / (2 * d))


def newton_with_kwargs(
        derivative, target_value, val0=300, valmin=70, valmax=3000, max_iter=10,
        tol_rel=ERR, tol_abs=ERR ** 2, tol_mode="rel", wrapper_func=None, **function_kwargs
    ):

    # start newton loop
    iteration = 0
    expr = True
    x = val0  # the initialized value in iteration
    parameter = function_kwargs["parameter"]  # the class of parameter
    function = function_kwargs["function"]
    relax = 1

    if tol_mode == "rel" and abs(target_value) <= 2 * tol_rel:  # the tol_mode means the type of iteration error accessed
        tol_mode = "abs"

    while expr:
        # calculate function residual and new value
        function_kwargs[parameter] = x  # refresh the value/key of parameters for the residual/derivative function in iteration
        residual = target_value - function(**function_kwargs)  # residual iteration
        x += residual / derivative(**function_kwargs) * relax  # iterate param value due to derivative

        # check for value ranges
        if x < valmin:  # the val min will be rewritten
            x = valmin
        if x > valmax:  # the val max will be rewritten
            x = valmax
        ...
        if iteration > max_iter:
            msg = (
                ...
                f'for function {function} in {wrapper_func}.\n ' + ' '*5 +
                f'Current value with x={x} is '
                f'{function(**function_kwargs)}, target value is '
                f'{target_value},\n ' + ' '*5 +
                f'residual is {residual} after {iteration} '
                'iterations.'
            )
            logger.debug(msg)
...
def get_basic_path():
    """
    Return the basic Aurora path and creates it if necessary.

    The basic path is the '.Aurora' folder in the $HOME directory.
    """
    basicpath = os.path.join(os.path.expanduser('~'), '.Aurora')
...
def extend_basic_path(subfolder):
    """
    Return a path based on the basic Aurora path and creates it if necessary.
