...
available from its original location Aurora/tools/logger.py
...
AURORA_LOGGER_ID = "AuroraLogger"
AURORA_PROGRESS_LOG_LEVEL = logging.INFO + 1  # 21
AURORA_RESULT_LOG_LEVEL = logging.INFO + 2  # 22

logging._levelToName[AURORA_PROGRESS_LOG_LEVEL] = 'PROGRESS'
logging._nameToLevel['PROGRESS'] = AURORA_PROGRESS_LOG_LEVEL
logging._levelToName[AURORA_RESULT_LOG_LEVEL] = 'RESULT'
logging._nameToLevel['RESULT'] = AURORA_RESULT_LOG_LEVEL

# Capture warnings globally instead of per file
logging.captureWarnings(True)
logger = logging.getLogger(AURORA_LOGGER_ID)
...
def progress(value, msg, *args, **kwargs):
    ...
    # We force the logging framework to trace past this file
    kwargs["stacklevel"] = increment_stacklevel(kwargs)
    return log(AURORA_PROGRESS_LOG_LEVEL, msg, *args, **kwargs)
...
def result(msg, *args, **kwargs):
    ...
    """
    kwargs["stacklevel"] = increment_stacklevel(kwargs)
    return log(AURORA_RESULT_LOG_LEVEL, msg, *args, **kwargs)
...
def add_console_logging(
    ...
    if log_the_version:
        info("Used AURORA version: {0}".format(get_version()))
...
def add_file_logging(
    ...
    ----------
    logpath : str
        The path for log files. By default a ".Aurora' folder is created in your
        home directory with subfolder called 'log_files'.

    logfile : str
        Name of the log file, default: Aurora.log
    ...
    logpath_setting = Aurora.tools.helpers.extend_basic_path('log_files')
    if logpath is not None:
        logpath_setting = logpath

    logfile_setting = os.path.join(logpath_setting, 'Aurora.log')
...
def define_logging(
        logpath=None, logfile='Aurora.log', file_format=None,
