...
Aurora/tools/fluid_properties/helpers.py
...
from Aurora.tools.helpers import central_difference
from Aurora.tools.helpers import newton_with_kwargs
from Aurora.tools.logger import logger
...
def get_molar_fractions(fluid_data):
    molarflow = {
        key: value["mass_fraction"] / value["wrapper"]._molar_mass  # molar fraction
...
def inverse_temperature_mixture(p=None, target_value=None, fluid_data=None, T0=None, f=None, wrapper_func=None, T_min=None):
    # calculate the fluid properties for fluid mixtures
    valmin, valmax = get_mixture_temperature_range(fluid_data)
    if T_min:
        valmin = max(valmin, T_min)
    ...
    return newton_with_kwargs(
        central_difference,  # derivative: discrete difference partial derivative
        target_value,  # iteration target
        val0=T0,
        valmin=valmin,
        valmax=valmax,
...
def darcy_friction_factor(re, ks, d):
    ...
    *transition zone and hydraulically rough:*
    ...
    >>> from Aurora.tools.fluid_properties.helpers import darcy_friction_factor
    >>> ks = 5e-5
    >>> d = 0.05
    >>> re_laminar = 2000
    >>> re_turb_smooth = 5000
